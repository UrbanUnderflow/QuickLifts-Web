...
3. **Activation Capsule Strip:** Two sub-capsules with explicit status:
   - **Lead:** “Hampton blueprint (secondary-only, Gate 2 partial, access locked).” Summary line: “Lead tile stays yellow because no primary access exists; best we have is one verified quote and Gate 2 partial.”
   - **Proof Lane:** “Oura/Fencing = research not started.” Summary line: “These lanes stay gray until real files drop (first file wins the flip).”
   Card explicitly states: “Proof over promises—no new partner wiring until files exist.” Includes quote, CTA copy, asset thumbnail, tags for “strategic lightness,” “loading the spring,” etc., inline badges (e.g., “Sentiment pending”) so gaps sit next to the quote slot, and a Story Update Note (“Baseline unchanged; story updated Oct 4”) so version deltas read like receipts; version tags only advance when real proof arrives (v1.0 = shippable, v1.1 = depth added) and must include the specific delta (“+2 quotes, cadence sample doubled”).

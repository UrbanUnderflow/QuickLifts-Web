...
async function generateThumbnail(videoUrl, exerciseVideoId) {
  functions.logger.info(`[Thumbnail] Starting generation for video: ${videoUrl}, ID: ${exerciseVideoId}`);
  ...
  if (videoUrl && videoUrl.startsWith('gs://')) {
      ...
      functions.logger.info(`[Thumbnail] Parsed gs:// URL. Bucket: ${bucketName}, Path: ${filePath}`);
      ...
      if (bucketIndex > 0 && pathIndex > 0 && pathIndex > bucketIndex) {
          ...
          functions.logger.info(`[Thumbnail] Parsed https:// URL. Bucket: ${bucketName}, Path: ${filePath}`);
      } else {
  ...
  try {
    ...
    functions.logger.info(`[Thumbnail] Downloading video from bucket ${bucketName}, path ${filePath} to ${tempLocalFile}`);
    await bucket.file(filePath).download({ destination: tempLocalFile });
    functions.logger.info(`[Thumbnail] Video downloaded successfully: ${tempLocalFile}`);

    // 2. Generate thumbnail using ffmpeg
    functions.logger.info(`[Thumbnail] Generating thumbnail at ${tempLocalThumb}`);
    await new Promise((resolve, reject) => {
      ffmpeg(tempLocalFile)
        .on('end', () => {
           functions.logger.info('[Thumbnail] FFmpeg processing finished.');
           resolve();
        })
        .on('error', (err, stdout, stderr) => {
          functions.logger.error('[Thumbnail] FFmpeg error:', err.message);
          functions.logger.error('[Thumbnail] FFmpeg stderr:', stderr);
    ...
    functions.logger.info(`[Thumbnail] Thumbnail generated: ${tempLocalThumb}`);

    // 3. Upload thumbnail to Storage
    functions.logger.info(`[Thumbnail] Uploading thumbnail to ${thumbStoragePath}`);
    ...
    functions.logger.info(`[Thumbnail] Thumbnail uploaded successfully.`);
    ...
    functions.logger.info(`[Thumbnail] Thumbnail public URL: ${publicUrl}`);
    return publicUrl;
    ...
    try {
      ...
      functions.logger.info('[Thumbnail] Cleaned up temporary files.');
    } catch (cleanupError) {
      functions.logger.error('[Thumbnail] Error cleaning up temp files:', cleanupError);
    }
  }
}

// --- Cloud Function: Automatic Thumbnail Generation --- 
exports.generateThumbnailOnWrite = functions.firestore
  .document('exerciseVideos/{exerciseVideoId}')
  .onWrite(async (change, context) => {
    const exerciseVideoId = context.params.exerciseVideoId;
    ...
    if (!dataAfter) {
      functions.logger.info(`[Thumbnail Trigger] Document ${exerciseVideoId} deleted. Skipping.`);
      ...
      //   functions.logger.info(`[Thumbnail Trigger] Deleted thumbnail for ${exerciseVideoId}.`);
      // } catch (deleteError) {
      //   if (deleteError.code !== 404) { // Ignore 'not found' errors
      //     functions.logger.error(`[Thumbnail Trigger] Failed to delete thumbnail for ${exerciseVideoId}:`, deleteError);
      //   }
      // }
      return null;
    }
    ...
    if (!dataAfter.videoURL) {
      functions.logger.info(`[Thumbnail Trigger] videoURL missing for ${exerciseVideoId}. Skipping.`);
      return null;
    }
    ...
    if (dataAfter.thumbnail) {
      ...
      if (!beforeThumbnail) {
        functions.logger.info(`[Thumbnail Trigger] Thumbnail was just added for ${exerciseVideoId}. Skipping.`);
        return null; 
      }
      ...
      if (dataBefore?.thumbnail && dataBefore?.videoURL === dataAfter.videoURL) {
          functions.logger.info(`[Thumbnail Trigger] Thumbnail exists and videoURL unchanged for ${exerciseVideoId}. Skipping.`);
          return null;
      }
      // Allow regeneration if videoURL changed but thumbnail didn't
      if (dataBefore?.videoURL !== dataAfter.videoURL) {
          functions.logger.info(`[Thumbnail Trigger] videoURL changed for ${exerciseVideoId}. Proceeding to regenerate thumbnail.`);
      } else {
          functions.logger.info(`[Thumbnail Trigger] Thumbnail exists for ${exerciseVideoId}. Skipping.`);
          return null;
      }
    }
    ...
    try {
      ...
      functions.logger.info(`[Thumbnail Trigger] Updating Firestore for ${exerciseVideoId} with thumbnail URL.`);
      ...
      functions.logger.info(`[Thumbnail Trigger] Firestore updated successfully for ${exerciseVideoId}.`);
      return null;

    } catch (error) {
      functions.logger.error(`[Thumbnail Trigger] Failed to generate thumbnail for ${exerciseVideoId}:`, error);
...
exports.generateMissingThumbnails = functions.https.onCall(async (data, context) => {
  // Optional: Add authentication/authorization check
  // if (!context.auth || !context.auth.token.admin) {
  //   throw new functions.https.HttpsError('unauthenticated', 'Only administrators can trigger this function.');
  // }

  functions.logger.info('[Manual Thumbnail Trigger] Starting queuing process.');
  ...
  try {
    ...
    functions.logger.info('[Manual Thumbnail Trigger] Fetching all exercise video documents...');
    const snapshot = await exerciseVideosRef.get();
    checkedCount = snapshot.size;
    functions.logger.info(`[Manual Thumbnail Trigger] Found ${checkedCount} total videos. Filtering...`);

    if (snapshot.empty) {
      functions.logger.info('[Manual Thumbnail Trigger] No exercise videos found in the collection.');
    ...
    snapshot.forEach(doc => {
      ...
      if (needsThumbnail && hasValidVideoUrl) {
        documentsToProcess.push(exerciseVideoId);
          ...
          //   functions.logger.debug(`[Manual Thumbnail Trigger] Skipping ${exerciseVideoId}: ${reason}`);
          // }
      }
    });
    ...
    if (queueCount === 0) {
      functions.logger.info('[Manual Thumbnail Trigger] No videos needed processing after filtering.');
      return { success: true, message: 'All videos already have thumbnails or have invalid URLs.', queuedCount: 0, skippedCount };
    }

    functions.logger.info(`[Manual Thumbnail Trigger] Queuing ${queueCount} videos for processing...`);
    ...
    functions.logger.error('[Manual Thumbnail Trigger] Error during queuing process:', error);
    throw new functions.https.HttpsError('internal', 'Failed to queue videos for thumbnail generation.', { queuedCount: 0, checkedCount, skippedCount });
  }
});

// --- Cloud Function: Process Thumbnail Queue (Pub/Sub Trigger) ---
// This function processes one video from the queue.
exports.processThumbnailQueue = functions.pubsub.topic(TOPIC_NAME)
  .onPublish(async (message, context) => {
    let exerciseVideoId;
    try {
      // Extract video ID from message payload
      const payload = message.json ? message.json : JSON.parse(Buffer.from(message.data, 'base64').toString());
      exerciseVideoId = payload.exerciseVideoId;

      if (!exerciseVideoId) {
        functions.logger.error('[Thumbnail Processor] Received message without exerciseVideoId.', message.json);
        return null; // Acknowledge the message to prevent retries
      }

      functions.logger.info(`[Thumbnail Processor] Received request for video ID: ${exerciseVideoId}`);
      ...
      if (!docSnap.exists) {
        functions.logger.warn(`[Thumbnail Processor] Document ${exerciseVideoId} does not exist. Skipping.`);
        return null;
      }
      ...
      if (videoData.thumbnail) {
        functions.logger.info(`[Thumbnail Processor] Thumbnail already exists for ${exerciseVideoId}. Skipping.`);
        return null;
      }
      ...
      if (!videoData.videoURL || !(videoData.videoURL.startsWith('gs://') || videoData.videoURL.startsWith('https://firebasestorage.googleapis.com'))) {
        functions.logger.warn(`[Thumbnail Processor] Invalid or missing videoURL for ${exerciseVideoId}. Skipping.`);
      ...
      functions.logger.info(`[Thumbnail Processor] Generating thumbnail for ${exerciseVideoId}...`);
      ...
      functions.logger.info(`[Thumbnail Processor] Successfully processed ${exerciseVideoId}. Thumbnail URL added.`);
      return null;

    } catch (error) {
      functions.logger.error(`[Thumbnail Processor] Failed to process video ID ${exerciseVideoId}:`, error);
      ...
      if (exerciseVideoId) { // Only update if we have an ID
          try {
              ...
              functions.logger.error(`[Thumbnail Processor] Failed to update error status for ${exerciseVideoId}:`, updateError);
          }
      }

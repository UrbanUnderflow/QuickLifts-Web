...
const AUDIT_CRITERIA: Record<string, string[]> = {
  ...
  advisor_nso_agreement: [
    ...
    'Exercise price tied to Board-determined fair market value, not corporate par value',
    'Fair-market-value determination date stated without an unsupported claim that a formal 409A appraisal exists',
    ...
    'Rule 701 service limitation: bona fide services exclude capital raising and securities-market promotion',
    'Early-exercise treatment is explicit; Section 83(b) notice appears only if unvested shares may be acquired',
  ...
  board_consent: [
    ...
    'Exercise price and Board-determined FMV are explicit and are not confused with corporate par value',
    'Vesting commencement date, cliff, total vesting period, and early-exercise treatment match the approved grant',
    'Effective date',
  ...
  eip: [
    ...
    'Plan reserve is an authorization, not an issuance; each award requires separate Board approval and an award agreement',
    'Eligibility definitions',
    'Award types included',
    'Stock option terms (ISOs and NSOs)',
    'Advisor/consultant Rule 701 eligibility limitations and per-grant securities-law review',
    'Exercise price uses fair market value rather than par value',
    'Early-exercise and Section 83(b) treatment is conditional and accurately timed',
...
const handler: Handler = async (event) => {
  ...
  try {
    const { documentId, content, documentType, title, requiresSignature } = JSON.parse(event.body || '{}');
    ...
    if (!response.ok) {
      ...
      return { statusCode: 500, headers, body: JSON.stringify({ error: 'Failed to audit document' }) };
    }

    const data = await response.json();
    const auditContent = data.choices[0]?.message?.content;
    if (!auditContent) {
      return { statusCode: 500, headers, body: JSON.stringify({ error: 'No audit result generated' }) };
    }
    ...
    try {
      ...
      return { statusCode: 500, headers, body: JSON.stringify({ error: 'Invalid audit response format' }) };
    }
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ success: true, audit: sanitizedAudit, documentId }),
    };
  } catch (error) {
    ...
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: error instanceof Error ? error.message : 'Failed to audit document' }),
    };
  }
};

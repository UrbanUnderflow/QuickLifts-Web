// =============================================================================
// AuntEDNA Status Webhook Receiver — clinical handoff operational mirror.
//
// Endpoint: POST /.netlify/functions/auntedna-callback
//
...
// buildPulseCallbackUrl in src/api/auntedna/client.ts). AuntEdna posts signed
// status events here; each event carries a unique `webhookEventId` and is
// retried until we acknowledge with a 2xx.
//
// Contract (admin page /admin/systemOverview#auntedna-escalation-data-exchange-contract):
//   escalation.created    -> store auntEdnaCaseId, status, receipt timestamp, event id
...
//   1. Verifies the HMAC-SHA256 signature against AUNTEDNA_WEBHOOK_SECRET.
//      Fails closed (503) when the secret is unset, unless AUNTEDNA_MOCK=true.
...
//      auntEdnaCaseId, status category, display-safe assignment label, and
...
//   5. Records a sanitized receipt in `pulsecheck-auntedna-webhook-events`
...
//     Non-2xx responses leave the event unclaimed so AuntEdna retries.
// =============================================================================

const crypto = require('crypto');
const { initializeFirebaseAdmin, admin, headers } = require('./config/firebase');

const WEBHOOK_EVENTS_COLLECTION = 'pulsecheck-auntedna-webhook-events';
...
function verifyWebhookSignature(event, rawBody) {
  const secret = String(process.env.AUNTEDNA_WEBHOOK_SECRET || '').trim();
  if (!secret) {
    // Clinical bridge fails closed: never accept unsigned partner traffic in
    // a real deployment. Mock mode (no live AuntEdna) is the only exception.
    if (process.env.AUNTEDNA_MOCK === 'true') {
...
function normalizeWebhookEvent(payload) {
  ...
  return {
    ...
    auntEdnaCaseId: pick('auntEdnaCaseId', 'caseId', 'escalationId'),
...
function buildEventDocId(webhookEventId) {
  return `auntedna_${webhookEventId.replace(/[/]/g, '_').slice(0, 480)}`;
}
...
async function resolveEscalationRef(db, webhookEvent) {
  ...
  if (webhookEvent.auntEdnaCaseId) {
    const snap = await db
      .collection(ESCALATION_RECORDS_COLLECTION)
      .where('clinicalReferenceId', '==', webhookEvent.auntEdnaCaseId)
      .limit(1)
      .get();
...
// behavior" column. Everything lands under the `auntEdnaCase` map so the
...
function buildEscalationMirror(webhookEvent, receivedAtSeconds) {
  ...
  const caseMirror = {
    caseId: webhookEvent.auntEdnaCaseId || null,
  ...
  const update = {
    auntEdnaCase: caseMirror,
    incidentLastActivityAt: receivedAtSeconds,
  };
  if (webhookEvent.auntEdnaCaseId) {
    update.auntEdnaCaseId = webhookEvent.auntEdnaCaseId;
  }
  return update;
}
  ...
  try {
    ...
    if (!verification.ok) {
      if (verification.mode === 'not_configured') {
        console.error('[auntedna-callback] AUNTEDNA_WEBHOOK_SECRET is not configured; rejecting webhook.');
        return json(503, {
          error: 'Webhook signing secret is not configured on the PulseCheck side.',
          errorCode: 'AUNTEDNA_WEBHOOK_SECRET_MISSING',
        });
      }
      return json(401, {
        error: 'Invalid AuntEdna webhook signature.',
        errorCode: 'AUNTEDNA_WEBHOOK_SIGNATURE_INVALID',
      });
    }

    const webhookEvent = normalizeWebhookEvent(parsePayload(rawBody));
    if (!webhookEvent.webhookEventId) {
      return json(400, { error: 'webhookEventId is required.', errorCode: 'AUNTEDNA_WEBHOOK_EVENT_ID_MISSING' });
    }
    if (!EVENT_STATUS_CATEGORY[webhookEvent.eventType]) {
      return json(400, {
        ...
        errorCode: 'AUNTEDNA_WEBHOOK_EVENT_TYPE_UNSUPPORTED',
      });
    }
    ...
    // leaves the event unclaimed so AuntEdna's retry can reprocess it.
    const outcome = await db.runTransaction(async (txn) => {
      ...
      txn.set(eventRef, {
        ...
        auntEdnaCaseId: webhookEvent.auntEdnaCaseId || null,
    ...
    if (!outcome.matched && !outcome.deduped) {
      console.warn(
        `[auntedna-callback] No escalation record matched event ${webhookEvent.webhookEventId} (pulseEscalationId=${webhookEvent.pulseEscalationId || 'none'}, auntEdnaCaseId=${webhookEvent.auntEdnaCaseId || 'none'}).`,
      );
    }
    ...
    console.error('[auntedna-callback] Failed:', error);
    const status = Number.isFinite(error?.statusCode) ? error.statusCode : 500;
    return json(status, {
      error: error?.message || 'AuntEdna webhook handling failed.',
      errorCode: 'AUNTEDNA_WEBHOOK_FAILED',
    });
  }
};

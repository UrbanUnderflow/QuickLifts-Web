...
export const handler: Handler = async (event) => {
  ...
  try {
    const {
      ...
      targetMonth,
      targetYear,
    } = JSON.parse(event.body || '{}') as ParseExpensesRequest;

    if (!Array.isArray(imageUrls) || imageUrls.length === 0) {
      return {
        statusCode: 400,
        headers: responseHeaders,
        body: JSON.stringify({ error: 'At least one image URL is required.' }),
      };
    }
    ...
    if (!apiKey) {
      return {
        statusCode: 500,
        headers: responseHeaders,
        body: JSON.stringify({ error: 'OpenAI API key not configured. Expected OPENAI_API_KEY or OPEN_AI_SECRET_KEY.' }),
      };
    }
...
Return JSON only in this exact shape:
{
  ...
  "summary": {
    ...
    console.error('[FounderBudget Netlify Function] Failed to parse screenshot expenses:', error);
    return {
      statusCode: 500,
      headers: responseHeaders,
      body: JSON.stringify({

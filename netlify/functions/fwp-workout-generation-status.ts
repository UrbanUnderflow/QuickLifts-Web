// FWP Workout Generation — status poller
// ----------------------------------------------------------------------------
// The iOS client polls this with the jobId until status is 'succeeded' (result
// is the raw OpenAI chat-completion) or 'failed'. Mirrors
// nora-routine-generation-status.
// ----------------------------------------------------------------------------
...
const verifyAuth = async (authHeader: string | undefined): Promise<string | null> => {
  ...
  try {
    ...
    console.error('[fwp-workout-generation-status] Auth verification failed:', error);
    return null;
  }
};
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      errorMessage: job.status === 'failed'
        ? `${job.errorMessage || "We couldn't build that workout right now. Try again in a moment."} Code: ${job.incidentId || job.errorCode || 'FWP_WORKOUT_GENERATION_FAILED'}.`
        : null,
    }),
  };
};

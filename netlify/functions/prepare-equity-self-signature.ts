...
export const handler: Handler = async event => {
  ...
  try {
    ...
    const result = await db.runTransaction(async transaction => {
      ...
      if (!Array.isArray(signers) || signers.length !== 1 || normalize(signers[0].email) !== normalize(identity.email) || normalize(signers[0].name) !== 'tremaine grant' || !signers[0].role?.trim()) throw fail(403, 'This document must name you, Tremaine Grant, as its sole authorized signer.');

...
const { CONNECTIONS_COLLECTION } = require('./polar-utils');
const {
  dateKeyInTimeZone,
  resolveTimeZone,
  shiftDateKey,
  syncPolarSnapshotForConnection,
} = require('./polar-sync');

const DEFAULT_LOCAL_HOUR = 2;
...
async function runScheduledPolarEndOfDaySync(now = new Date()) {
  const db = admin.firestore();
  const targetLocalHour = Number(process.env.POLAR_EOD_SYNC_LOCAL_HOUR || DEFAULT_LOCAL_HOUR);
  const limit = Number(process.env.POLAR_EOD_SYNC_LIMIT || DEFAULT_LIMIT);
  ...
  for (const doc of snapshot.docs) {
    ...
    const userId = connection.userId || doc.id;
    ...
    try {
      ...
      await db.collection(CONNECTIONS_COLLECTION).doc(doc.id).set({
        ...
        lastEndOfDaySyncError: error?.message || 'Polar end-of-day sync failed',
      }, { merge: true });
      results.push({ userId, status: 'error', snapshotDateKey, timezone, error: error?.message || String(error) });
      console.warn('[scheduled-polar-end-of-day-sync] user sync failed', userId, error);
    }
  }
...
exports.handler = schedule('15 * * * *', async (event) => {
  try {
    ...
    console.error('[scheduled-polar-end-of-day-sync] fatal', error);
    return {
      statusCode: 500,
      body: JSON.stringify({ ok: false, error: error?.message || 'Polar end-of-day sync failed' }),
    };
  }
});

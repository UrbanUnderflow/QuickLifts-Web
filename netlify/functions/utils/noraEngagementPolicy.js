...
function buildNoraBoundaryResponse(lane, { athleteMessage = '', category = '' } = {}) {
  ...
  if (lane === NoraConversationLane.ClinicalCare) {
    if (/\b(?:medication|prescription|medicine)\b/i.test(canonicalAthleteMessage) && /\b(?:remember|save|store|notes|next time)\b/i.test(canonicalAthleteMessage)) return 'Keep medication details in your care provider’s secure record. For questions about the medication, your prescriber or pharmacist can help. We can continue with your performance question here without adding medication details to your mental notes.';

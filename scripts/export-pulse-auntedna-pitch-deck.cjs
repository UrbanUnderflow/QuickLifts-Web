...
const EXPORT_DIR = path.join(ROOT_DIR, 'exports', 'pulse-auntedna-pitch');
const SLIDES_DIR = path.join(EXPORT_DIR, 'slides');
const OUTPUT_PDF = path.join(EXPORT_DIR, 'pulse-auntedna-pitch-deck.pdf');
...
const exportDeck = async () => {
  ...
  try {
    await waitForServer(`${baseUrl}/pulse-auntedna-pitch`);
    ...
    await page.goto(`${baseUrl}/pulse-auntedna-pitch`, { waitUntil: 'networkidle', timeout: 120000 });

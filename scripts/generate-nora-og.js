/* eslint-disable */
// One-off script: render the Nora splash to a static PNG at
// /public/nora-og.png. Mirrors scripts/generate-pil-og.js so the /Nora
// route has a matching-style preview image with its own wordmark.
// Run locally on macOS so sharp/librsvg can pick up real system fonts
// (SF Pro, Helvetica).
//
// Usage: node scripts/generate-nora-og.js
...
const svg = `<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="600" y="315"
        ...
        letter-spacing="8">Nora</text>
</svg>`;

const outPath = path.join(__dirname, '..', 'public', 'nora-og.png');
...
sharp(Buffer.from(svg))
  ...
  .catch((err) => {
    console.error('✗ Failed to render Nora OG:', err);
    process.exit(1);
  });

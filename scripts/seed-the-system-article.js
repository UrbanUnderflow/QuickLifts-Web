/**
 * One-time script to seed "the-system" article into the researchArticles
 * Firestore collection so it shows up on the /research listing page.
 *
 * Usage:  FIREBASE_SECRET_KEY="..." node scripts/seed-the-system-article.js
 *
 * The FIREBASE_SECRET_KEY should be the same one used by the rest of the app.
 * It will be in your .env or Netlify env variables.
 */
...
if (!admin.apps.length) {
    ...
    if (!secretKey) {
        console.error('❌  FIREBASE_SECRET_KEY env var is required.');
        console.error('   Run with: FIREBASE_SECRET_KEY="your-key" node scripts/seed-the-system-article.js');
        console.error('   Or: source .env && node scripts/seed-the-system-article.js');
        process.exit(1);
    }
...
const articleData = {
    ...
    status: 'published',
    createdAt: admin.firestore.Timestamp.fromDate(new Date('2026-02-05T00:00:00.000Z')),
    updatedAt: admin.firestore.Timestamp.fromDate(new Date('2026-02-05T00:00:00.000Z')),
    publishedAt: admin.firestore.Timestamp.fromDate(new Date('2026-02-05T00:00:00.000Z')),
};

// ─── Seed the article ──────────────────────────────────────────────
async function seed() {
    try {
        const docRef = db.collection('researchArticles').doc(ARTICLE_SLUG);
        const existing = await docRef.get();

        if (existing.exists) {
            console.log(`⚠️  Document "researchArticles/${ARTICLE_SLUG}" already exists.`);
            console.log('   Overwriting with updated data...');
        }

        await docRef.set(articleData);
        console.log(`✅  Article "${ARTICLE_SLUG}" saved to researchArticles collection.`);
        console.log('   It should now appear on /research listing page.');
        process.exit(0);
    } catch (err) {

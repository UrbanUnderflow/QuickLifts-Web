...
export const PILOT_DASHBOARD_METRIC_EXPLANATIONS = {
  ...
  'recommendation-type-slices': {
    title: 'Recommendation Type Slices',
    whatItMeans: 'This groups persisted recommendation projections by consumer so the dashboard can show which audience types the engine is currently serving.',
    whyItMatters: 'It helps separate athlete-facing, coach-facing, and operational outputs instead of flattening all projections into one count.',
    howToReadIt: 'Read these counts as a distribution check. If one consumer dominates, that may be fine, but it is important to know the shape of the output mix before drawing conclusions.',
    watchFor: 'If a consumer slice is missing entirely, the current dashboard frame may be under-instrumented for that audience or the backend segmentation may still be incomplete.',
  },
  'active-cohorts': {
  ...
  'athlete-trust': {
    title: 'Athlete Trust',
    whatItMeans: 'This is the average athlete trust score for the currently selected pilot frame.',
    whyItMatters: 'Athlete trust tells you whether the experience feels believable, safe, and useful to the people using the product.',
    howToReadIt: 'If the sample is small, treat the score as directional only and look directly at the response count before drawing conclusions.',
    watchFor: 'A low sample can create a deceptively sharp-looking score, so always check the count behind the number.',
  },
  'coach-trust': {
    title: 'Coach Trust',
    whatItMeans: 'This is the average coach trust score for the currently selected pilot frame.',
    whyItMatters: 'Coach trust helps show whether the guidance feels dependable to the people who operationalize the pilot day to day.',
    howToReadIt: 'Use the response count to judge whether the score is truly representative or still too thin to trust.',
    watchFor: 'A small number of coach responses can make the average swing more than it should.',
  },
  'clinician-trust': {
    title: 'Clinician Trust',
    whatItMeans: 'This is the average clinician trust score for the currently selected pilot frame.',
    whyItMatters: 'Clinician trust matters when the pilot has care-adjacent workflows or clinically informed escalation paths.',
    howToReadIt: 'Look for the minimum sample signal before treating the score as stable.',
    watchFor: 'If only one or two clinicians responded, the number is probably not ready for interpretation yet.',
  },
  'athlete-nps': {
    title: 'Athlete NPS',
    whatItMeans: 'This is the average athlete recommendation score for the currently selected pilot frame.',
    whyItMatters: 'It shows whether athletes would recommend the experience after using it in the pilot.',
    howToReadIt: 'Low sample counts can distort the result, so always compare the score to the response count before using it in a readout.',
    watchFor: 'A score without enough responses should stay in “not enough responses yet” posture.',
  },
  'coach-nps': {
    title: 'Coach NPS',
    whatItMeans: 'This is the average coach recommendation score for the currently selected pilot frame.',
    whyItMatters: 'It shows whether coaches would recommend the pilot workflow or guidance experience to another coach.',
    howToReadIt: 'Treat the response count as part of the metric, not just as metadata.',
    watchFor: 'Thin samples can hide a lot of noise here.',
  },
  'clinician-nps': {
    title: 'Clinician NPS',
    whatItMeans: 'This is the average clinician recommendation score for the currently selected pilot frame.',
    whyItMatters: 'It shows whether clinicians would recommend the pilot workflow or guidance experience to another clinician.',
    howToReadIt: 'Check the sample size before comparing the score across pilots or cohorts.',
    watchFor: 'If the sample is below threshold, keep the dashboard in low-confidence posture.',
  },
  'readiness-frame': {

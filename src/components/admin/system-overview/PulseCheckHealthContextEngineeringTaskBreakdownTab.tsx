...
const M2_ROWS = [
  ...
  ['Validate parity against current live behavior', 'Ensure shared Fit With Pulse context is still visible after cutover.', 'QA + PulseCheck iOS', 'Regression protection.'],
];
...
const M3_ROWS = [
  ...
  ['Validate standalone-athlete path', 'Confirm PulseCheck can produce meaningful context without Fit With Pulse usage.', 'QA + PulseCheck iOS', 'Standalone milestone.'],
];
...
const QA_ROWS = [
  ['Bridge-only athlete', 'Athlete uses Fit With Pulse data only, no native PulseCheck HealthKit yet.', 'Must remain strong through Milestone 2.'],
  ['Standalone native athlete', 'Athlete never uses Fit With Pulse and only connects PulseCheck-native HealthKit.', 'Critical for Milestone 3.'],
...
const HANDOFF_CARDS = [
  ...
  },
  {
    ...
    body: 'PulseCheck can generate useful standalone context from native HealthKit records even when the athlete does not use Fit With Pulse.',
  },
];

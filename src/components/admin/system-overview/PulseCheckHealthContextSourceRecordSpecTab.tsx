...
const PRINCIPLE_CARDS = [
  {
    ...
    accent: 'red' as const,
    body: 'HealthKit, Oura, QuickLifts, and self-report adapters should emit normalized source records only. They should not create Nora-facing summaries or consumer-specific payloads.',
  },
  {
...
const RECORD_ROWS = [
  ...
  ['sourceFamily', 'One of `quicklifts`, `healthkit`, `apple_watch`, `oura`, `pulsecheck_self_report`, or future approved families.', 'Required.'],
  ['sourceType', 'The concrete adapter or feed name, such as `healthkit_sleep`, `oura_readiness`, or `quicklifts_workout_summary`.', 'Required.'],
...
const TYPE_ROWS = [
  ...
  ['checkin', 'PulseCheck-native subjective response such as mood, readiness, stress, confidence, or post-session reflection.', 'Behavioral and emotional context.'],
  ['summary_input', 'A pre-aggregated upstream record allowed as an input to assembly, such as QuickLifts daily summary fragments.', 'Migration-safe bridge from current system to target system.'],
...
const PAYLOAD_ROWS = [
  ['training payload', 'Workout title, type, duration, calories, total volume, sets, reps, exercises, body parts, source workout id.', 'QuickLifts and HealthKit session records.'],
  ...
  ['nutrition payload', 'Meal id, calories, macros, hydration, nutrient detail, journal confidence, meal timestamp.', 'Journal-entry or daily-total inputs.'],
  ...
  ['behavioral payload', 'Mood, readiness, stress, perceived recovery, sim outcome, compliance, notes, selected answers.', 'PulseCheck-native checkins and reflections.'],
];

const SOURCE_ROWS = [
  ['QuickLifts / FitWithPulse adapter', 'Emit training, nutrition, and approved summary-input records from the shared app ecosystem.', 'May preserve richer workout semantics than HealthKit.'],
  ...
  ['PulseCheck self-report adapter', 'Emit checkin and behavioral records from PulseCheck-native flows.', 'Subjective context belongs in the same record system, not a side channel.'],
];
...
const HANDOFF_STEPS = [
  {
    ...
    body: 'The adapter reads a HealthKit sample, Oura payload, QuickLifts workout, or PulseCheck checkin event.',
...
const BUILD_ORDER_ROWS = [
  ...
  ['3. Implement PulseCheck self-report adapter', 'Write checkins and behavioral context into the same source-record system.', 'Makes subjective context a first-class input.'],
  ...
  return (
    <div className="space-y-10">
      ...
      <RuntimeAlignmentPanel
        ...
        masterReference="Use this page before implementing native HealthKit ingestion, Oura ingestion, QuickLifts snapshot migration adapters, PulseCheck self-report ingestion, or operator tooling for health-context debugging."
        relatedDocs={[

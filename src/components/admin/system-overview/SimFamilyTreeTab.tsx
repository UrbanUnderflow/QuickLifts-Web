...
const LOCKED_FAMILIES: LockedFamily[] = [
    {
        ...
        variants: ['Visual disruption Reset', 'Audio disruption Reset', 'Cognitive-provocation Reset', 'Combined-channel Reset', 'Short daily Reset (3–5 min)', 'Extended Trial Reset (10–15 min)', 'Sport-context Reset (basketball, tennis, etc.)', 'Immersive Reset Chamber (Vision Pro football package)'],
    },
    {
        name: 'Noise Gate',
        pillar: 'focus',
    ...
    },
    {
        name: 'Signal Window',
        pillar: 'decision',
        mechanism: 'Signal discrimination under time pressure',
        ...
        variants: ['Ambiguous Signal Window', 'Decoy Signal Window', 'Shot-clock Signal Window', 'Rapid recognition Signal Window', 'Spatial read Signal Window', 'Field-read Trial Signal Window', '3D Spatial Read (Vision Pro football package)'],
    },
    {
        name: 'Sequence Shift',
        pillar: 'decision',
...
const CANDIDATE_FAMILIES: CandidateFamily[] = [
    ...
    },
    {
        ...
        description: 'Signal Window = reading the signal correctly. Blind Commit = committing when the read is inherently incomplete. Separates confident-but-wrong from accurate-but-hesitant — both actionable coaching insights.',
    ...
    },
    {
        ...
        promotionCriteria: 'Distinct from signal-discrimination metrics; novel predictive value for pre-action performance quality',
...
const HYBRID_MODULES: HybridModule[] = [
    ...
    { name: 'Two-Minute Drill', families: 'Signal Window + Brake Point + Heat Check', description: 'Compressed-duration, high-consequence module combining signal reading, inhibition, and pressured performance.' },
...
const VARIANT_LIBRARY: VariantGroup[] = [
    ...
    { parent: 'Brake Point', pillar: 'decision', isCandidate: false, variants: ['Red Light (classic go/no-go at sport tempo)', 'False Start (prevent premature initiation under anticipation pressure)', 'False Key (high-temptation decoy before real signal)', 'Decoy Lane (spatial decoys pull toward wrong action)', 'Spatial Brake (cancel response to spatial decoy, Vision Pro-ready)'] },
    { parent: 'Signal Window', pillar: 'decision', isCandidate: false, variants: ['Snap Read (rapid correct choice from short signal window)', 'Split Second (compressed-time decision with speed-accuracy balance)', 'Shot Clock (decision under shrinking time window)', 'Window Close (correct option briefly available, rewarding controlled recognition)', 'Check Down (choose safer correct option over tempting decoy)', 'Spatial Read (3D signal discrimination, Vision Pro football package)'] },
    ...
    { parent: 'Split Stream', pillar: 'focus', isCandidate: true, variants: ['Multi-Source (signals from multiple directions/channels)', 'Priority Stack (multiple active rules, choose by current priority)', 'Dual-channel monitoring (two simultaneous task streams)', 'Rotating-source load (relevant channel changes unpredictably)'] },
    ...
    { parent: 'Chaos Read', pillar: 'decision', isCandidate: true, variants: ['Reset and Read (disruption then immediate decision)', 'Blindside (unexpected signal from unattended channel)', 'Noise to Choice (filtering task transitions into decision task)', 'Compressed chaos (maximum information density, minimum time)'] },

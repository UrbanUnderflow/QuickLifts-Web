...
export const ClubMemberApp: React.FC<ClubMemberAppProps> = ({
  ...
  // Effect to update activeMessage if the underlying message changes in consolidatedMessages
  useEffect(() => {
    if (!activeMessage) return;
    const message = consolidatedMessages.find(m => m.id === activeMessage.id);
    // console.log('Active Message Updated', { msgId: message.id, activeMessageId: activeMessage?.id });
  ...
  <div className="min-h-screen overflow-y-auto font-sans text-white" style={{ backgroundColor: darkBg }}>
    ...
    {activeSurvey && (
      <SurveyTakingModal
        ...
        survey={activeSurvey}
        onSubmit={async (answers, respondentName, respondentEmail, responseId) => {
          if (!activeSurvey || !activeMessage) return;
          const instanceId = activeSurveyInstanceId;
          ...
          try {
            if (activeMessage && currentUser) {
              ...
              updatedCompletedBy[currentUser.id] = responseId;

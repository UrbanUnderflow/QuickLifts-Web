...
const shouldUseDevProject = process.env.NEXT_PUBLIC_E2E_FORCE_DEV_FIREBASE === 'true';

const getFirebaseConfig = () => {
  if (shouldUseDevProject) {
    return {
...
const getFallbackApp = (): FirebaseApp => {
...
export async function getFirestoreDocFallback(collectionName: string, documentId: string) {
  if (!documentId) {
    return null;
  }

  const app = getFallbackApp();

...
const INVEST_PASSCODE = 'PULSE';
const ADMIN_PASSCODE = 'ONEUP';
const INVEST_UNLOCK_KEY = 'invest-unlocked';
const INVEST_PASSCODE_USED_KEY = 'invest-passcode-used';

const Invest = ({ metaData }: InvestProps) => {
  ...
  useEffect(() => {
    try {
      ...
      const unlocked = sessionStorage.getItem(INVEST_UNLOCK_KEY) === 'true';
      const used = sessionStorage.getItem(INVEST_PASSCODE_USED_KEY) as 'PULSE' | 'ONEUP' | null;
  ...
  const handlePasscodeSubmit = (e: React.FormEvent) => {
    ...
    if (trimmed === INVEST_PASSCODE) {
      try {
        sessionStorage.setItem(INVEST_UNLOCK_KEY, 'true');
        sessionStorage.setItem(INVEST_PASSCODE_USED_KEY, 'PULSE');
      } catch {
        // ignore
      }
      ...
      try {
        sessionStorage.setItem(INVEST_UNLOCK_KEY, 'true');
        sessionStorage.setItem(INVEST_PASSCODE_USED_KEY, 'ONEUP');
      } catch {
        // ignore
      }
  ...
  const getAnalyticsHeaders = async (): Promise<Record<string, string>> => {
    ...
    if (passcodeUsed === 'ONEUP') {
      return { 'X-Invest-Passcode': 'ONEUP' };
    }
    return {};
  };
  ...
  const fetchAnalyticsList = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/invest/analytics', { headers });
  ...
  const fetchAnalyticsDetail = async (ip: string) => {
    ...
    try {
      const headers = await getAnalyticsHeaders();
      const res = await fetch(`/api/invest/analytics?ip=${encodeURIComponent(ip)}`, { headers });
  ...
  if (!isUnlocked) {
    return (
      <div className="bg-zinc-950 text-white min-h-screen flex flex-col items-center justify-center px-6">
        <PageHead 
          metaData={metaData}
          pageOgUrl="https://fitwithpulse.ai/Invest"
          pageOgImage="/pulse-invest-og.png"
        />
        <img src="/PulseWhite.png" alt="Pulse" className="h-14 w-auto mb-6" />
        <h1 className="text-xl font-bold text-white mb-1">Investor Deck</h1>
  ...
  return (
    <div className="bg-zinc-950 text-white min-h-screen">
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-zinc-700">
              {analyticsDetailIp ? (
                ...
                <h2 id="analytics-modal-title" className="text-lg font-bold text-white">Investor Deck Analytics</h2>
              )}
              <button
      ...
      >
        ...
        >
          <div className="relative z-10 w-full max-w-6xl mx-auto px-6">
            <h2 className={`${isMobile ? 'text-3xl' : 'text-4xl md:text-5xl'} font-bold mb-4 text-white text-center animate-fade-in-up`}>
              Who's building <span className="text-[#E0FE10]">Pulse</span>?
            </h2>
            
            {/* Core Team */}
            <div className="mb-10 animate-fade-in-up animation-delay-150">
              <p className="text-zinc-400 text-xs uppercase tracking-widest mb-6 text-center">Core Team</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <a href={linkedIn.tremaine} target="_blank" rel="noopener noreferrer" className="group">
                  <GlassCard>
                    <div className="p-6 text-center">
                      <div className="w-20 h-20 rounded-full bg-[#E0FE10]/20 flex items-center justify-center mx-auto mb-4 group-hover:bg-[#E0FE10]/30 transition-colors">
                        <span className="text-3xl">👤</span>
                      </div>
                      <h3 className="text-white font-bold text-lg">Tremaine Grant</h3>
                      <p className="text-[#E0FE10] text-sm">CEO & Founder</p>
                    </div>
                  </GlassCard>
                </a>
                
                <a href={linkedIn.bobby} target="_blank" rel="noopener noreferrer" className="group">
                  <GlassCard>
                    <div className="p-6 text-center">
                      <div className="w-20 h-20 rounded-full bg-[#E0FE10]/20 flex items-center justify-center mx-auto mb-4 group-hover:bg-[#E0FE10]/30 transition-colors">
                        <span className="text-3xl">👤</span>
                      </div>
                      <h3 className="text-white font-bold text-lg">Bobby Nweke</h3>
                      <p className="text-[#E0FE10] text-sm">Chief of Staff</p>
                    </div>
                  </GlassCard>
                </a>
                
                <a href={linkedIn.lola} target="_blank" rel="noopener noreferrer" className="group">
                  <GlassCard>
                    <div className="p-6 text-center">
                      <div className="w-20 h-20 rounded-full bg-[#E0FE10]/20 flex items-center justify-center mx-auto mb-4 group-hover:bg-[#E0FE10]/30 transition-colors">
                        <span className="text-3xl">👤</span>
                      </div>
                      <h3 className="text-white font-bold text-lg">Lola Oluwaladun</h3>
                      <p className="text-[#E0FE10] text-sm">Design Lead</p>
                    </div>
                  </GlassCard>
                </a>
              </div>
            ...
            <div className="animate-fade-in-up animation-delay-300">
              <p className="text-zinc-400 text-xs uppercase tracking-widest mb-6 text-center">Advisors</p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { name: 'Marques Zak', role: 'CMO @ ACC', link: linkedIn.marques },
                  { name: 'Valerie Alexander', role: 'Fortune 500 Consultant', link: linkedIn.valerie },
                  { name: 'DeRay Mckesson', role: 'Campaign Zero', link: linkedIn.deray },
                  { name: 'Erik Edwards', role: 'Partner @ Cooley', link: linkedIn.erik },
                ].map((advisor, index) => (
                  <a key={index} href={advisor.link} target="_blank" rel="noopener noreferrer" className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800 hover:border-[#E0FE10]/30 transition-colors text-center">
                    <p className="text-white font-medium text-sm">{advisor.name}</p>
                    <p className="text-zinc-500 text-xs">{advisor.role}</p>
                  </a>
                ))}
              </div>
            </div>
...
export const getServerSideProps: GetServerSideProps<InvestProps> = async (_context) => {
  let rawMetaData: FirestorePageMetaData | null = null;
  try {
    rawMetaData = await adminMethods.getPageMetaData('Invest');
  } catch (error) {
    console.error("Error fetching page meta data for Invest page:", error);
  }

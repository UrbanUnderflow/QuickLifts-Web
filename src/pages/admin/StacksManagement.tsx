...
const StacksManagement: React.FC = () => {
  ...
  const renderStackExerciseDetails = () => {
    if (!selectedStack || !showDetailsModal) return null;
        ...
        <div className="bg-[#1a1e24] rounded-xl p-6 shadow-xl border border-[#d7ff00]/30 max-w-4xl w-full max-h-[90vh] flex flex-col">
          <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-semibold text-white">Stack Details: {selectedStack.title}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div className="bg-[#262a30] rounded-lg p-4 border border-gray-700">
              <h4 className="text-white font-semibold mb-2 flex items-center gap-2"><Info className="h-4 w-4" /> Overview</h4>
              <div className="text-sm text-gray-300 space-y-1">
                <p><span className="text-gray-400">ID:</span> {selectedStack.id}</p>
                <p><span className="text-gray-400">Author:</span> {selectedStack.author || 'Unknown'}</p>
                <p><span className="text-gray-400">User ID:</span> {selectedStack.userId || 'N/A'} {selectedStack.userId && (<button onClick={()=>handleCopyToClipboard(selectedStack.userId!, 'User ID')} className="ml-2 text-xs text-blue-400 underline">Copy</button>)}</p>
                <p><span className="text-gray-400">Zone:</span> {selectedStack.zone}</p>
                <p><span className="text-gray-400">Duration:</span> {selectedStack.duration ? `${selectedStack.duration} min` : 'N/A'}</p>
                <p><span className="text-gray-400">Type:</span> {selectedStack.autoGenerated ? 'AI Generated' : 'User Created'}</p>
                <p><span className="text-gray-400">Source Round:</span> {selectedStack.sourceRoundId || 'N/A'} {selectedStack.sourceRoundId && (<button onClick={()=>handleCopyToClipboard(selectedStack.sourceRoundId!, 'Round ID')} className="ml-2 text-xs text-blue-400 underline">Copy</button>)}</p>
                <p><span className="text-gray-400">Created:</span> {formatDate(selectedStack.createdAt)}</p>
                <p><span className="text-gray-400">Updated:</span> {formatDate(selectedStack.updatedAt)}</p>
              </div>
            </div>
            <div className="bg-[#262a30] rounded-lg p-4 border border-gray-700">
              <h4 className="text-white font-semibold mb-2">Description</h4>
              <p className="text-sm text-gray-300 whitespace-pre-line">{selectedStack.description || 'No description'}</p>
            </div>
          </div>
                    ...
                    <tr key={stack.id} className="hover:bg-[#20252c] transition-colors">
                      ...
                      <td className="py-4 px-5 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => {
                              setSelectedStack(stack); setShowDetailsModal(true); fetchStackExercises(stack.id);
                            }}
                          ...
                          <span className="px-2 py-1 rounded-md text-xs bg-zinc-700/70 text-zinc-300">{stack.exerciseCount} ex</span>
                        </div>
                      </td>

          ...
          <div className="relative bg-[#1a1e24] rounded-xl p-6 mb-6 shadow-xl overflow-hidden">
            ...
            <div className="flex flex-wrap justify-between items-end gap-4 mb-6">
              ...
              <div className="flex gap-2 items-center">
                 ...
                 {!isSelectingForDelete && filteredUsers.length === 1 && (
                   <button
                     onClick={() => handleDeleteUser(filteredUsers[0])}
                     disabled={isBatchDeleting || processingDelete === filteredUsers[0].id}
                     className={`bg-red-700/80 text-white px-4 py-3 rounded-lg font-medium transition flex items-center text-sm
                       ${(isBatchDeleting || processingDelete === filteredUsers[0].id) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-600/80'}`}
                     title={`Delete ${filteredUsers[0].username || filteredUsers[0].email || filteredUsers[0].id}`}
                   >
                     {processingDelete === filteredUsers[0].id ? (
                     ...
                     {processingDelete === filteredUsers[0].id ? 'Deleting...' : 'Delete User'}
                   </button>
                 )}
                      ...
                      <React.Fragment key={user.id}>
                        <tr className={`hover:bg-[#2a2f36] transition-colors ${selectedUser?.id === user.id ? 'bg-[#1d2b3a]' : ''}`}>
                          ...
                          <td className="py-3 px-4 border-b border-gray-700">
                            ...
                            <td className="py-3 px-4 border-b border-gray-700">
                              ...
                              <button
                                ...
                                className="px-2 py-1 rounded-md text-xs font-medium transition-colors bg-red-900/30 text-red-300 hover:bg-red-900/50 border border-red-900 disabled:opacity-50 disabled:cursor-not-allowed"
                                title={`Delete ${user.username || user.email || user.id}`}
                              >
                                {processingDelete === user.id ? (
                                  <svg className="animate-spin h-3 w-3 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                          ...
                          <td className="py-3 px-4 border-b border-gray-700 text-center">
	                        ...
	                        {selectedUser?.id === user.id && (
	                                  <tr>
	                              <td colSpan={isSelectingForDelete ? 19 : 18} className="p-0 border-b border-gray-700">

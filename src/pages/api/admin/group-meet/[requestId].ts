...
async function buildRequestDetail(
  ...
  return {
    ...
    finalSelection: (requestData.finalSelection || null) as GroupMeetFinalSelection | null,
    calendarInvite: (requestData.calendarInvite || null) as GroupMeetCalendarInvite | null,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      return res.status(200).json({ request: await buildRequestDetail(requestRef, requestDoc) });
    } catch (error: any) {
      console.error('[group-meet-admin-detail] Failed to load request:', error);
      return res.status(500).json({ error: error?.message || 'Failed to load Group Meet request.' });
    }
  }
  ...
  try {
    ...
    const timezone = (body.timezone || requestData.timezone || 'America/New_York').trim() || 'America/New_York';
    const deadlineRaw =
      typeof body.deadlineAt === 'string' ? body.deadlineAt : toIso(requestData.deadlineAt) || '';
    const deadline = new Date(deadlineRaw);
    const targetMonth = String(requestData.targetMonth || '').trim();
    const meetingDurationMinutes = Math.max(
      15,
      Math.min(240, Number(body.meetingDurationMinutes) || Number(requestData.meetingDurationMinutes) || 30)
    );
    ...
    if (!isValidGroupMeetMonth(targetMonth)) {
      return res.status(400).json({ error: 'This Group Meet request is missing a valid target month.' });
    }
    ...
    const durationChanged =
      meetingDurationMinutes !== Math.max(15, Number(requestData.meetingDurationMinutes) || 30);
    const timezoneChanged = timezone !== (requestData.timezone || 'America/New_York');
    ...
    return res.status(500).json({ error: error?.message || 'Failed to update Group Meet request.' });
  }
}

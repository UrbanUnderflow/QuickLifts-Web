  ...
  try {
    ...
    if (!recipientEmail) {
      return res.status(400).json({
        error: "This participant does not have an email address on file.",
      });
    }
    ...
    if (!result.success) {
      return res
        .status(500)
        .json({ error: emailError, invite, status: nextStatus });
    }
    ...
    return res
      .status(500)
      .json({ error: error?.message || "Failed to resend invite." });
  }
}

...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to load Group Meet requests." });
    }
  }
  ...
  try {
    ...
    const timezone =
      (body.timezone || "America/New_York").trim() || "America/New_York";
    const meetingDurationMinutes = Math.max(
      15,
      Math.min(240, Number(body.meetingDurationMinutes) || 30),
    );
    ...
    if (!isValidGroupMeetMonth(targetMonth)) {
      return res
        .status(400)
        .json({ error: "A valid target month is required." });
    }
    ...
    if (!hostContactId) {
      return res
        .status(400)
        .json({ error: "Choose the host from your contact list." });
    }
    ...
    if (!hostContactSnap.exists) {
      return res
        .status(400)
        .json({ error: "The selected host contact could not be found." });
    }
    ...
    const normalizedHost = {
      ...
      availabilityEntries: normalizeGroupMeetAvailabilitySlots(
        body.host?.availabilityEntries,
        targetMonth,
      ),
    };
    ...
    const participantContactSnapshots = await Promise.all(
      requestedParticipantIds.map((contactId) =>
        contactsRef.doc(contactId).get(),
      ),
    );

    const missingParticipantContact = participantContactSnapshots.find(
      (snapshot) => !snapshot.exists,
    );
    if (missingParticipantContact) {
      return res.status(400).json({
        error: "One or more selected guest contacts could not be found.",
      });
    }
    ...
    if (!dedupedParticipants.length) {
      return res
        .status(400)
        .json({ error: "Choose at least one guest from your contact list." });
    }
    ...
    const createdInvites: Array<{
      ...
      availabilityEntries: ReturnType<
        typeof normalizeGroupMeetAvailabilitySlots
      >;
    }> = [];
    ...
    const request: GroupMeetRequestSummary = {
      ...
      responseCount: createdInvites.filter(
        (invite) => invite.summary.respondedAt,
      ).length,
      status: "draft",
    ...
    return res.status(500).json({
      error: error?.message || "Failed to create Group Meet request.",
    });
  }
}

...
const COLLECTION = 'wunnaRunViews';
const ADMIN_COLLECTION = 'admin';

const ONEUP_SECRET = process.env.WUNNA_ONEUP_SECRET || 'ONEUP';
...
async function requireAdmin(req: NextApiRequest): Promise<{ email?: string } | null> {
  ...
  const passcode = req.headers['x-wunna-run-passcode'] as string | undefined;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.error('[wunna-run/analytics] Error:', error);

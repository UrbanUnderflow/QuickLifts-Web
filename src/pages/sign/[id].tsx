...
const SignDocument: React.FC = () => {
  ...
  const getDocumentHtmlContent = (downloadSignedVersion: boolean) => {
    if (!request) return '';
    ...
    if (request.documentContent) {
      if (request.documentContent.includes('<!DOCTYPE html>')) {
        ...
        if (downloadSignedVersion) {
          // Add a "SIGNED" stamp to the invoice if it's an HTML invoice
          html = html.replace('</div>\n  <div class="company-badge">',
            `  <div style="background: #10b981; color: white; padding: 8px 16px; border-radius: 8px; font-size: 12px; font-weight: 700; margin-right: 12px; letter-spacing: 0.5px;">SIGNED ✓</div>\n</div>\n  <div class="company-badge">`);
        }
        return html;
      } else {
        // Standard markdown document
        return generateGenericHtml(request.documentName, request.documentContent, currentDate, signatureName, signatureStyle, company, downloadSignedVersion);
      }
    }

    // Fallbacks for legacy documents
    return request.documentType === 'advisor'
      ? generateAdvisorAgreementHtml(agreementName, currentDate, signatureName, signatureStyle, company, downloadSignedVersion)
      : generateContractorAgreementHtml(agreementName, currentDate, signatureName, signatureStyle, company, downloadSignedVersion);
  };
  ...
  const generateGenericHtml = (title: string, content: string, date: string, signature: string, signatureStyle: string, company: string, isSigned: boolean) => `
    <!DOCTYPE html>
    <html>
      <head>
        <title>${isSigned ? 'Signed - ' : ''}${title}</title>
        <style>
          @page { margin: 1in; }
          ...
          .company-name { font-size: 14pt; font-weight: bold; margin-bottom: 4px; text-transform: uppercase; }
      ...
      <body>
        <div class="header">
          ...
          <div class="document-date">${isSigned ? 'Signed: ' + date : 'Generated on: ' + date}</div>
        </div>
        <h1>${title} ${isSigned ? '<span class="signed-badge">✓ SIGNED</span>' : ''}</h1>
        <div class="content">
        ...
        <div class="signature-block">
          ...
          <div style="display: flex; justify-content: space-between; flex-wrap: wrap;">
            <div style="width: 45%;">
              <div class="signature-line" style="${isSigned ? signatureStyle : ''}">${isSigned ? 'Tremaine Grant' : ''}</div>
            ...
            <div style="width: 45%;">
              <div class="signature-line" style="${isSigned ? signatureStyle : ''}">${isSigned ? signature : ''}</div>
  ...
  const generateAdvisorAgreementHtml = (name: string, date: string, signature: string, signatureStyle: string, company: string, isSigned: boolean) => `
    <!DOCTYPE html>
    <html>
      <head>
        <title>${isSigned ? 'Signed - ' : ''}Independent Advisor Agreement</title>
        <style>
      ...
      <body>
        <h1>Independent Advisor / Trial Contractor Agreement ${isSigned ? '<span class="signed-badge">✓ SIGNED</span>' : ''}</h1>
        
        <p class="intro">
          This Independent Advisor / Trial Contractor Agreement (the "Agreement") is entered into as of <span class="highlight">${date}</span> (the "Effective Date"), by and between <span class="highlight">${company}</span>, a Delaware corporation (the "Company"), and <span class="highlight">${name}</span> ("Advisor").
        </p>
        ...
        <div class="signature-section">
          ...
          <div class="signature-block">
            <p class="signature-name">${company}</p>
            <div class="signature-line" style="${isSigned ? signatureStyle : ''}">${isSigned ? 'Tremaine Grant' : ''}</div>
            <p>By: Tremaine Grant</p>
            <p class="signature-title">Title: CEO</p>
            <div class="date-row">
              <span class="date-label">Date:</span>
              <span class="date-value">${isSigned ? date : ''}</span>
            </div>
          </div>
  ...
  const generateContractorAgreementHtml = (name: string, date: string, signature: string, signatureStyle: string, company: string, isSigned: boolean) => `
    <!DOCTYPE html>
    <html>
      <head>
        <title>${isSigned ? 'Signed - ' : ''}Independent Contractor Agreement</title>
        <style>
          body { font-family: 'Times New Roman', Times, serif; padding: 50px 60px; color: #111; margin: 0; line-height: 1.6; font-size: 14px; }
          h1 { font-size: 18px; text-align: center; margin-bottom: 20px; font-weight: bold; text-transform: uppercase; letter-spacing: 1px; }
          .intro { margin-bottom: 30px; text-align: justify; }
      ...
      <body>
        <h1>Independent Contractor Agreement ${isSigned ? '<span class="signed-badge">✓ SIGNED</span>' : ''}</h1>
        
        <p class="intro">
          This Independent Contractor Agreement (the "Agreement") is entered into as of <span class="highlight">${date}</span> (the "Effective Date"), by and between <span class="highlight">${company}</span>, a Delaware corporation (the "Company"), and <span class="highlight">${name}</span> ("Contractor").
        </p>
        ...
        <div className="bg-zinc-900 border-b border-zinc-800 py-4 px-6">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              {request.companyName?.includes('TresProperties') ? (
                <span className="text-xl font-bold text-white tracking-widest uppercase items-center flex h-8">TRESPROPERTIES</span>
              ) : (
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 mb-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg text-sm transition-colors whitespace-nowrap"
                >
                  <Download className="w-4 h-4" />
                  Download PDF
                </button>
              </div>
              <div className="bg-white rounded-xl overflow-hidden shadow-inner border border-zinc-200" style={{ height: '65vh' }}>
                <iframe
                  srcDoc={getDocumentHtmlContent(false)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {['font-dancing', 'font-vibes', 'font-pacifico'].map((font, index) => (
                      <button
                        ...
                        className={`p-4 rounded-xl border-2 transition-all ${selectedFont === index

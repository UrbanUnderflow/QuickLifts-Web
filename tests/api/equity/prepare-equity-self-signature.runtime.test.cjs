...
for(const status of ['failed','deferred']) test(`reuses a current request after legacy email ${status} without replacing signatures`,async()=>{const s=setup();const first=JSON.parse((await s.invoke()).body);const saved=s.rows.get(`signingRequests/${first.delivery.documentId}`);saved.status=status;saved.emailStatus='blocked';const writes=s.writes();const result=await s.invoke();assert.equal(result.statusCode,200,result.body);assert.equal(JSON.parse(result.body).delivery.documentId,first.delivery.documentId);assert.equal(s.writes(),writes);});

test('an authorized admin can prepare delivery to the saved director email without changing the signer',async()=>{const s=setup({award:{preparedSigners:[{name:'Tremaine Grant',email:'saved-director@example.com',role:'Sole director'}]}});const first=await s.invoke();assert.equal(first.statusCode,200,first.body);const d=JSON.parse(first.body).delivery;assert.equal(d.recipientEmail,'saved-director@example.com');const request=s.rows.get(`signingRequests/${d.documentId}`);assert.equal(request.recipientEmail,'saved-director@example.com');assert.equal(request.status,'pending');assert.equal(request.signatureData,undefined);const again=await s.invoke();assert.equal(again.statusCode,200,again.body);assert.equal(JSON.parse(again.body).delivery.documentId,d.documentId);});

...
test('firebase-next-api exposes the full migrated route count and matches dynamic patterns', () => {
  ...
  assert.equal(
    proxyModule.__test.resolveRoutePattern('/api/admin/nora-guard/revoke'),
  ...
  assert.equal(
    proxyModule.__test.resolveRoutePattern('/api/nora-notetaker/sync-bot'),
    '/api/nora-notetaker/sync-bot'
  );
  assert.equal(
  ...
  assert.equal(
    ...
    '/api/group-meet/host-selection/[token]'
  );
  assert.equal(
